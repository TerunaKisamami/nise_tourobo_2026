...
# アクションインターフェースのインポート
from auto_robot_interfaces.action import (OverSteps, BoxArm, PoseCorrection,
...
class RobotActionClient(Node):
    ...
    def __init__(self):
        ...
        self.map_path = "/home/aratahorie/NHK2026_auto_robot_v2/src/auto_robot_v2/map/c419_map_1.yaml.yaml"
        ...
        self.spear_init_pos = [0.9991, 1.1486, 1.595]
        self.aruco_init_pos = [0.7638, 0.7688, -1.676]
        self.step_0_init_pos = [0, 0, 0]
        self.step_1_init_pos = [0, 0, 0]
        self.step_2_init_pos = [8.0696, 0.2564, 0.0723]
    ...
    async def steps_action_pattern(self, num):
        ...
        if num == 1:
            ...
            await self.send_to_boxarm(11)
            await self.send_to_move_on_steps(7)

        elif num == 5:
            #正面取得 ,右腕、下
            await self.send_to_move_on_steps(4)
            await self.send_to_boxarm(7)
            await self.send_to_move_on_steps(7)

        elif num == 6:
            #正面取得 ,右腕、上
            await self.send_to_move_on_steps(4)
            await self.send_to_boxarm(3)
            await self.send_to_move_on_steps(7)

        elif num == 7:
            #正面取得 ,左腕、同じ
            await self.send_to_move_on_steps(4)
            await self.send_to_boxarm(9)
            await self.send_to_move_on_steps(7)

        elif num == 8:
            #正面取得 ,左腕、下
            await self.send_to_move_on_steps(4)
            await self.send_to_boxarm(5)
            await self.send_to_move_on_steps(7)

        elif num == 9:
            #正面取得 ,左腕、上
            await self.send_to_move_on_steps(4)
            await self.send_to_boxarm(1)
            await self.send_to_move_on_steps(7)
    ...
    async def run_robot_sequence(self):
        ...
        #段差超え(テスト)
        #await self.steps_action_pattern(30)  #スタート、段差超え
        #await self.steps_action_pattern(24)  #位置確認、真ん中移動
        #await self.steps_action_pattern(18)
        #await self.steps_action_pattern(2)
        ...
        ##Vゴールシークエンス
        #await self.send_goal(self.step_2_to_after)  #棚の前まで移動
        #await self.send_to_boxarm(15)  #中段を入れる
        await self.send_to_is_vgoal()  #高さ待ち
        await self.send_to_boxarm(16)  #上段を入れる
